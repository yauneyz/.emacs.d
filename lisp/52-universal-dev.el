...
(defun +dev/build ()
  ...
  (pcase major-mode
    ('go-mode (+go/build))
    ('go-ts-mode (+go/build))
    ('clojure-mode (cider-eval-string "(compile 'user)"))
    ('python-mode (compile "python -m build"))
    ('rust-mode (compile "cargo build"))
...
(defun +dev/run ()
  ...
  (pcase major-mode
    ('go-mode (+go/run))
    ('go-ts-mode (+go/run))
    ('clojure-mode (message "Use REPL to run Clojure code"))
    ('python-mode (compile "python -m main"))
    ('rust-mode (compile "cargo run"))
...
(defun +dev/clean ()
  ...
  (interactive)
  (pcase major-mode
    ('go-mode (compile "go clean ./..."))
    ('go-ts-mode (compile "go clean ./..."))
    ('clojure-mode (compile "lein clean"))
    ('python-mode (compile "rm -rf build/ dist/ *.egg-info/"))
    ('rust-mode (compile "cargo clean"))
